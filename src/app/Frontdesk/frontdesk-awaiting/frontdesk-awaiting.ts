...
@Component({
  selector: 'app-frontdesk-awaiting',
  imports: [CommonModule],
  templateUrl: './frontdesk-awaiting.html',
  styleUrl: './frontdesk-awaiting.scss'
})
export class FrontdeskAwaiting implements OnInit {
  ...
  async ngOnInit() {
    this.route.queryParamMap.subscribe(async (params) => {
      const frontdeskId = params.get('frontdeskId');
      this.schoolId = params.get('schoolId');
      const schoolId = this.schoolId;
      if (!frontdeskId || !schoolId) {
        this.loading.set(false);
        return;
      }
      try {
        const ref = doc(this.firestore, `schools/${schoolId}/frontdesks/${frontdeskId}`);
  ...
  goToSchoolDashboard() {
    ...
    if (v) {
      this.router.navigate([`/FrontdeskDashboard/${v}`]);
    } else if (this.schoolId) {
      // Fallback: if no code/slug, try using schoolId
      this.router.navigate([`/FrontdeskDashboard/${this.schoolId}`]);
    } else {
