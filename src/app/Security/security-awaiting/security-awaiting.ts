...
@Component({
  selector: 'app-security-awaiting',
  imports: [CommonModule],
  templateUrl: './security-awaiting.html',
  styleUrl: './security-awaiting.scss'
})
export class SecurityAwaiting implements OnInit {
  ...
  async ngOnInit() {
    this.route.queryParamMap.subscribe(async (params) => {
      ...
      if (!securityId || !schoolId) {
        this.loading.set(false);
        return;
      }
      try {
        const ref = doc(this.firestore, `schools/${schoolId}/securitys/${securityId}`);
