...
@Component({
  selector: 'app-itdepartment-awaiting',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './itdepartment-awaiting.html',
  styleUrl: './itdepartment-awaiting.scss'
})
export class ItdepartmentAwaiting implements OnInit {
  ...
  async ngOnInit() {
    this.route.queryParamMap.subscribe(async (params) => {
      const itdepartmentId = params.get('itdepartmentId');
      this.schoolId = params.get('schoolId');
      const schoolId = this.schoolId;
      if (!itdepartmentId || !schoolId) {
        this.loading.set(false);
        return;
      }
      try {
        const ref = doc(this.firestore, `schools/${schoolId}/itdepartments/${itdepartmentId}`);
