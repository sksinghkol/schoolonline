...
@Component({
  selector: 'app-parrent-menu',
  imports: [RouterLink, CommonModule],
  templateUrl: './parrent-menu.html',
  styleUrl: './parrent-menu.scss'
})
export class ParrentMenu implements OnInit {
  ...
  parrent: DocumentData | null = null;
  ...
  async ngOnInit() {
    ...
    onAuthStateChanged(this.firebaseAuth, async (firebaseUser) => {
      ...
      if (firebaseUser && this.school) {
        const parrentRef = doc(this.firestore, `schools/${this.school['id']}/parrents/${firebaseUser.uid}`);
        const parrentSnap = await getDoc(parrentRef);
        if (parrentSnap.exists()) {
          const parrentData = parrentSnap.data();
          if (firebaseUser.photoURL && (!parrentData['photoURL'] || parrentData['photoURL'] !== firebaseUser.photoURL)) {
            await updateDoc(parrentRef, { photoURL: firebaseUser.photoURL });
            parrentData['photoURL'] = firebaseUser.photoURL;
          }
          this.parrent = parrentData;
        } else {
          this.parrent = null;
        }
      } else {
