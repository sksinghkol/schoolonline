...
@Component({
  selector: 'app-itdepartment-menu',
  standalone: true,
  imports: [RouterLink, CommonModule],
  templateUrl: './itdepartment-menu.html',
  styleUrl: './itdepartment-menu.scss'
})
export class ItdepartmentMenu implements OnInit {
  ...
  itdepartment: DocumentData | null = null; // Changed from student to director
  ...
  async ngOnInit() {
    ...
    onAuthStateChanged(this.firebaseAuth, async (firebaseUser) => {
      ...
      if (firebaseUser && this.school) {
        const itdepartmentRef = doc(this.firestore, `schools/${this.school['id']}/itdepartments/${firebaseUser.uid}`);
        const itdepartmentSnap = await getDoc(itdepartmentRef);
        if (itdepartmentSnap.exists()) {
          const itdepartmentData = itdepartmentSnap.data();
          if (firebaseUser.photoURL && (!itdepartmentData['photoURL'] || itdepartmentData['photoURL'] !== firebaseUser.photoURL)) {
            await updateDoc(itdepartmentRef, { photoURL: firebaseUser.photoURL });
            itdepartmentData['photoURL'] = firebaseUser.photoURL;
          }
          this.itdepartment = itdepartmentData;
        } else {
