...
@Component({
  ...
  templateUrl: './transport-login.html',
  styleUrl: './transport-login.scss'
})
export class TransportLogin implements OnInit {
    ...
    if (schoolSlugFromUrl) {
      ...
      setTimeout(() => {
        if (!this.schoolState.currentSchool()) {
          ...
          this.schoolLoadError.set(`School not found for “${this.selectedSchoolSlug}”. Try: /transport-login/${normalized} or check the school slug.`);
        }
      ...
      this.schoolLoadError.set('No school provided in the URL. Use /transport-login/{schoolSlug}');
      this.resolving.set(false);
    }
  }
  ...
  private async checkStaffStatus(user: User) {
    ...
    try {
      ...
      const ref = doc(this.firestore, `schools/${this.selectedSchool.id}/transports/${user.uid}`);
      ...
      if (!snap.exists()) {
        try {
          await setDoc(ref, {
            uid: user.uid,
            email: user.email,
            name: user.displayName || this.loginForm.value.name || 'New Transport Staff',
        ...
        this.router.navigate(['/transport-awaiting'], {
          queryParams: { transportId: user.uid, schoolId: this.selectedSchool.id }
        });
        return;
      }

      const transportData = snap.data() as any;
      if (!transportData?.uid) {
        try { await updateDoc(ref, { uid: user.uid }); } catch {}
      }

      switch (transportData?.status) {
        case 'approved':
