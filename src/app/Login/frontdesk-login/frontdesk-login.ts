...
@Component({
  ...
  templateUrl: './frontdesk-login.html',
  styleUrl: './frontdesk-login.scss'
})
export class FrontdeskLogin implements OnInit {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (user && school?.id) {
        ...
        this.checkFrontdeskStatus(user);
      } else if (school?.id) {
        this.selectedSchool = school;
      }
    });
    ...
    if (schoolSlugFromUrl) {
      ...
      setTimeout(() => {
        if (!this.schoolState.currentSchool()) {
          ...
          this.schoolLoadError.set(`School not found for “${this.selectedSchoolSlug}”. Try: /frontdesk-login/${normalized} or check the school slug.`);
        }
      ...
      this.schoolLoadError.set('No school provided in the URL. Use /frontdesk-login/{schoolSlug}');
      this.resolving.set(false);
    }
  }
  ...
  private async checkFrontdeskStatus(user: User) {
    ...
    try {
      ...
      const ref = doc(this.firestore, `schools/${this.selectedSchool.id}/frontdesks/${user.uid}`);
      ...
      if (!snap.exists()) {
        try {
          await setDoc(ref, {
            uid: user.uid,
            email: user.email,
            name: user.displayName || this.loginForm.value.name || 'New Frondesk Staff',
            photoURL: user.photoURL || null,
            createdAt: serverTimestamp(),
            status: 'waiting-approval'
          }, { merge: true });
        } catch (e) {
          console.warn('Failed to create initial staff doc', e);
        }
        this.router.navigate(['/frontdesk-awaiting'], {
          queryParams: { frontdeakId: user.uid, schoolId: this.selectedSchool.id }
        });
        return;
      }

      const frontdeskData = snap.data() as any;
      if (!frontdeskData?.uid) {
        try { await updateDoc(ref, { uid: user.uid }); } catch {}
      }

      switch (frontdeskData?.status) {
        case 'approved':
        ...
        case 'waiting-approval':
          this.router.navigate(['/frontdesk-awaiting'], {
            queryParams: { frontdeskId: user.uid, schoolId: this.selectedSchool.id }
          });
          break;
        default:
          try { await updateDoc(ref, { status: 'waiting-approval' }); } catch {}
          // Navigate to transport awaiting page for unhandled statuses
          this.router.navigate(['/frontdesk-awaiting'], {
            queryParams: { frontdeskId: user.uid, schoolId: this.selectedSchool.id }
          });
      }
  ...
  private async saveLoginHistory(staffId: string) {
