...
@Component({
  selector: 'app-itdepartment-login',
  imports: [CommonModule, ReactiveFormsModule, RouterLink],
  templateUrl: './itdepartment-login.html',
  styleUrl: './itdepartment-login.scss'
})
export class ItdepartmentLogin implements OnInit {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (user && school?.id) {
        ...
        this.checkItdepartmentStatus(user);
      } else if (school?.id) {
        this.selectedSchool = school;
      }
    });
    ...
    if (schoolSlugFromUrl) {
      ...
      setTimeout(() => {
        if (!this.schoolState.currentSchool()) {
          ...
          this.schoolLoadError.set(`School not found for “${this.selectedSchoolSlug}”. Try: /itdepartment-login/${normalized} or check the school slug.`);
        }
      ...
      this.schoolLoadError.set('No school provided in the URL. Use /itdepartment-login/{schoolSlug}');
      this.resolving.set(false);
    }
  }
  ...
  private async checkItdepartmentStatus(user: User) {
    ...
    try {
      ...
      const ref = doc(this.firestore, `schools/${this.selectedSchool.id}/itdepartments/${user.uid}`);
      ...
      if (!snap.exists()) {
        try {
          await setDoc(ref, {
            uid: user.uid,
            email: user.email,
            name: user.displayName || this.loginForm.value.name || 'New IT Department Staff',
            photoURL: user.photoURL || null,
            createdAt: serverTimestamp(),
            status: 'waiting-approval'
          }, { merge: true });
        } catch (e) {
          console.warn('Failed to create initial IT Department doc', e);
        }
        this.router.navigate(['/itdepartment-awaiting'], {
          queryParams: { itdepartmentId: user.uid, schoolId: this.selectedSchool.id }
        });
        return;
      }

      const itdepartmentData = snap.data() as any;
      if (!itdepartmentData?.uid) {
        try { await updateDoc(ref, { uid: user.uid }); } catch {}
      }

      switch (itdepartmentData?.status) {
        case 'approved':
          await this.saveLoginHistory(user.uid);
          this.router.navigate(['/itdepartment-dashboard'], {
            queryParams: { itdepartmentId: user.uid, schoolId: this.selectedSchool.id }
          });
          break;
  ...
  private async saveLoginHistory(itdepartmentId: string) {
