...
@Component({
  ...
  styleUrl: './security-login.scss'
})
export class SecurityLogin implements OnInit {
    ...
    if (schoolSlugFromUrl) {
      ...
      setTimeout(() => {
        if (!this.schoolState.currentSchool()) {
          ...
          this.schoolLoadError.set(`School not found for “${this.selectedSchoolSlug}”. Try: /security-login/${normalized} or check the school slug.`);
        }
      ...
      this.schoolLoadError.set('No school provided in the URL. Use /security-login/{schoolSlug}');
      this.resolving.set(false);
    }
  }
  ...
  private async checkSecurityStatus(user: User) {
    ...
    try {
      ...
      const ref = doc(this.firestore, `schools/${this.selectedSchool.id}/securitys/${user.uid}`);
      ...
      if (!snap.exists()) {
        try {
          await setDoc(ref, {
            uid: user.uid,
            email: user.email,
            name: user.displayName || this.loginForm.value.name || 'New Security Staff',
        ...
        this.router.navigate(['/security-awaiting'], {
          queryParams: { securityId: user.uid, schoolId: this.selectedSchool.id }
        });
        return;
      }

      const securityData = snap.data() as any;
      if (!securityData?.uid) {
        try { await updateDoc(ref, { uid: user.uid }); } catch {}
      }

      switch (securityData?.status) {
        case 'approved':
