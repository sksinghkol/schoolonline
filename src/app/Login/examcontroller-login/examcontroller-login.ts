...
@Component({
  selector: 'app-examcontroller-login',
  imports: [CommonModule, ReactiveFormsModule, RouterLink],
  templateUrl: './examcontroller-login.html',
  styleUrl: './examcontroller-login.scss'
})
export class ExamcontrollerLogin implements OnInit {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (user && school?.id) {
        ...
        this.checkExamcontrollerStatus(user);
      } else if (school?.id) {
        this.selectedSchool = school;
      }
    });
    ...
    if (schoolSlugFromUrl) {
      ...
      setTimeout(() => {
        if (!this.schoolState.currentSchool()) {
          ...
          this.schoolLoadError.set(`School not found for “${this.selectedSchoolSlug}”. Try: /examcontroller-login/${normalized} or check the school slug.`);
        }
      ...
      this.schoolLoadError.set('No school provided in the URL. Use /examcontroller-login/{schoolSlug}');
      this.resolving.set(false);
    }
  }
  ...
  private async checkExamcontrollerStatus(user: User) {
    ...
    try {
      ...
      const ref = doc(this.firestore, `schools/${this.selectedSchool.id}/examcontrollers/${user.uid}`);
      ...
      if (!snap.exists()) {
        try {
          await setDoc(ref, {
            uid: user.uid,
            email: user.email,
            name: user.displayName || this.loginForm.value.name || 'New Exam Controller',
            photoURL: user.photoURL || null,
            createdAt: serverTimestamp(),
            status: 'waiting-approval'
          }, { merge: true });
        } catch (e) {
          console.warn('Failed to create initial exam controller doc', e);
        }
        this.router.navigate(['/examcontroller-awaiting'], {
          queryParams: { examcontrollerId: user.uid, schoolId: this.selectedSchool.id }
        });
        return;
      }

      const examcontrollerData = snap.data() as any;
      if (!examcontrollerData?.uid) {
        try { await updateDoc(ref, { uid: user.uid }); } catch {}
      }

      switch (examcontrollerData?.status) {
        case 'approved':
          await this.saveLoginHistory(user.uid);
          this.router.navigate(['/examcontroller-dashboard'], {
            queryParams: { examcontrollerId: user.uid, schoolId: this.selectedSchool.id }
          });
          break;
